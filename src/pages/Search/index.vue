            ...
            <div class="navbar-inner filter">
              ...
              <ul class="sui-nav">
                <li :class="{active:isOne}" @click="changeOrder('1')">
                  <a>综合<span v-show="isOne" class="iconfont" :class="{ 'icon-direction-down-circle': isDesc,'icon-up1':isAsc }"></span></a>
                </li>
                <li :class="{active:isTwo}" @click="changeOrder('2')">
                  <a>价格<span v-show="isTwo" class="iconfont" :class="{'icon-direction-down-circle': isDesc,'icon-up1':isAsc}"></span></a>
                </li>
              </ul>
            </div>
          </div>
...
export default {
  ...
  data() {
    return {
      searchParams: {
        ...
        "pageNo": 1,//分页器：代表是当前是第几页
        "pageSize": 3,//代表的是每一页展示数据的个数
        "props": [],//平台售卖属性操作带的参数
        "trademark": "", //品牌
    }
   } 
  ...
  computed: {
    //mapGetters里面的写法：传递的数组，因为getters计算是没有划分模块的【即没有home、search之分】
  ...
  methods: {
    ...
    removeCategoryName() {
      ...
      //带给服务器参数都是可有可无的：如果属性值为空的字符串还是会把相应的字段带给服务器
      //但是如果把相应的字段变为undefined，则当前这个字段不会带给服务器
      ...
      //严谨：本意是删除query，如果路径当中出现params则不应该删除，路由跳转的时候应该带着
  ...
  watch: {
    //监听属性:监听路由的信息是否发生变化，如果发生变化，则再次发起请求
    $route(newValue,oldValue) {
      //再次发请求之前整理带给服务器参数
      Object.assign(this.searchParams, this.$route.query, this.$route.params);
      //再次发起ajax请求
      this.getData();
      //每一次请求完毕，应该把相应的1、2、3级分类的id置空的，让它接受下一次的相应的1、2、3id
      //为什么分类名字与关键字不用清理：因为每一次路由发生变化的时候，都会给它赋予新的数据
