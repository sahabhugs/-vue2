...
export default {
  ...
  methods: {
    ...
    handler:throttle(async function(type, disNum, cart) {
      //type：为了区分这三个元素
      //目前disNum这个形参：+ 变化量(1)  - 变化量(-1)  input 最终的个数（并不是变化量）
      //cart:哪一个产品【身上有id】
      switch(type) {
        //加号
        case "add":
        ...
        //如果出现产品的个数小于或等于1，传递给服务器的个数为0，原封不动
          disNum = cart.skuNum > 1 ? -1 : 0;
          break;
        case "change":
        //用户输入进来的最终量，如果非法的（带有汉字或者出现负数），带给服务器数字零
    ...
    async updateChecked(cart, event) {
      //带给服务器的参数isChecked,不应该是布尔值，而应该是0或1----所以需要对其进行判断
      try {
  ...
  computed: {
    ...
    //判断底部复选框是否勾选【全部产品都选中时，才勾选】
    isAllCheck() {
      //遍历数组里面的元素，只要全部元素isChecked属性都为1，则返回为真true
      //只要有一个不是1,则返回为假false
