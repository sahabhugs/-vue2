<template>
    <div class="pagination">
        ...
        v-show=" page >= startNumAndEndNum.start" @click="$emit('getPageNo', page)"
...
export default {
    ...
    computed: {
        ...
        startNumAndEndNum() {
            ...
            //先定义两个变量存储起始数字与结束数字
            let start = 0, end = 0;
            //连续页码数字5【就是至少五页】，如果出现不正常的现象【页码数不够五页】
            //不正常现象：总页数没有连续页码多
            if (continues > totalPage) {
                ...
                //正常现象：连续页码是5，总页数一定是大于5的
                //parse（）取整
                //起始数字
                start = pageNo - parseInt(continues / 2);
                //结束数字
                end = pageNo + parseInt(continues / 2);
                // 把出现不正常的现象【start数字出现0或负数的情况】纠正
                if (start < 1) {
                    start = 1;
                    end = continues;  
                }
                //把出现不正常的现象【end数字大于总页码的情况】纠正
                if (end > totalPage) {
                    end = totalPage;
